% fp(0,0,1,0,1,2,2,0.01,6,1E-15);
% fp(0,0,1,0,1,2,50,0.01,6,1E-15);
% fp(0,0,1,0,1,2,30,0.01,6,1E-15);
function [] = singlecell_usual(n_in, m_in, h_in,v_input,I_start_time, I_end_time,I_induct_value,dt,tmax,tolerance)
...
plot(time_axis,h1,'.b',time_axis,h2,'.k');
       legend('INduct current sum','Added current sum');
       title(sprintf('Time step %d of %d at time %d s', i, N,(i*dt)));
       drawnow;
    ...
    legend('Leaking current','Compare current','Added curent');
    title(sprintf('Time step %d of %d at time %d s', i, N,(i*dt)));
    drawnow;
end
end
