...
%
%This function disfunction [] = HW02_Problem_02(v0, theta, dt, B, m,range_approx_tolorance)
%function [x_normal, y_normal, x_airdrag, y_airdrag, x_density, y_densifunction [] = HW02_Problem_02(v0, theta, dt, B, m,range_approx_tolorance)
%function [x_normal, y_normal, x_airdrag, y_airdrag, x_density, y_density, Py] = HW02_Problem_02(v0, theta, dt, B, m)
%
...
%hold on;
%plot(x_density, y_density, 'g.-', )
plot(x_normal,y_normal,'r-^', x_airdrag, y_airdrag,'b-o',x_density,y_density, 'g.-',xmax, yax2, 'c.-');
...
%}
    ...
    k=k+1;
rangecom(k)=range2;
if  (k>5)&&abs(rangecom(k)-rangecom(k-1))>range_approx_tolorance
...
elseif  (k>5)&&abs(rangecom(k)-rangecom(k-1))<=range_approx_tolorance
    if mod(k,2)==1
    theta2=(theta2+theta_max2)/2;
    else%if mod(k,3)==0
%    theta2=theta_max2-abs(theta_max2+theta2)/2;
